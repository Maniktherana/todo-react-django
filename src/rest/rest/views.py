...
class TodoListView(APIView):
    ...
    def get(self, request):
        try:
            ...
            return Response({'error': 'Internal Server Error', "hello": os.environ["DATABASE_NAME"] }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def post(self, request):
        try:
            ...
            if todo_item:
                result = self.mongo_connection.collection.insert_one({'todo': todo_item})
                inserted_id = str(result.inserted_id)
                return Response({'id': inserted_id, 'todo': todo_item}, status=status.HTTP_201_CREATED)
            else:
                return Response({'error': 'Request data must contain a todo item'}, status=status.HTTP_400_BAD_REQUEST)
        except Exception as e:
